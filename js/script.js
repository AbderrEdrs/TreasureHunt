...
application.Database =	function() {
	this.database = openDatabase('Treasure_hunt', '1.0', 'Treasure Hunt', 10000);
};

application.Database.prototype = {
	
	clean : function(){
		this.database.transaction(function(tx){tx.executeSql('DROP TABLE question');});
		this.database.transaction(function(tx){tx.executeSql('DROP TABLE hunt');});
	},

	init :	function(){
		this.database.transaction(function(t){t.executeSql('CREATE TABLE IF NOT EXISTS hunt(id INTEGER PRIMARY KEY AUTOINCREMENT, name)');});
		this.database.transaction(function(t){t.executeSql('CREATE TABLE IF NOT EXISTS question(id INTEGER PRIMARY KEY AUTOINCREMENT, question, code, id_hunt)');});
	},
...
application.Row = function (_screen, _name, _parent, parameters) {
    ...
    domHelp.addText(td_nb_questions,(parameters.nb_questions?parameters.nb_questions:""));

	var td_action=domHelp.addElement(newLine,"td");
	var buttonActionRepondre = new application.Button(screen, "answerButton", td_action, "Répondre", "btn btn-success");
	var buttonActionDelete = new application.Button(screen, "deleteHunt", td_action, "Supprimer", "btn btn-danger");

    buttonActionRepondre.getButton().addEventListener('click', function() {
        var ws = new application.Database();
        var db = ws.getdatabase();
        var id = parameters.hunt_id;
        db.readTransaction(function(tx){
            tx.executeSql('SELECT * FROM hunt INNER JOIN question ON hunt.id = question.id_hunt where hunt.id = '+id, [],
                function (tx, results) {
	...
	buttonActionDelete.getButton().addEventListener('click', function(){
        var ws = new application.Database();
        var db = ws.getdatabase();
        var id = parameters.hunt_id;

        db.transaction(function(tx){
			tx.executeSql('DELETE FROM hunt WHERE id = '+id);
			tx.executeSql('DELETE FROM question where id_hunt = '+id, [], window.location.reload());
		})
    });
	...
	this.__defineGetter__("name", function () {return name;});
	this.__defineSetter__("name", function (newName) {name=newName;});
...
application.Screen.prototype = {
	addTitle : function (_titleText) {
		...
		domHelp.addText(this.titleNode,_titleText);
	},
	...
	addButtonCreateHunt : function (name, label) {
		...
		new_hunt_btn.getButton().addEventListener('click', function(){
			var screen = new application.Screen();
			screen.cleanShowDiv();
	...
	show_div_create_hunt: function (){
		...
		buttonCreate.getButton().addEventListener('click', function(){
            ...
            if(textField.value.length >0){
                ...
                alert("Le nom du hunt ne doit pas être vide");
        });

		buttonCancel.getButton().addEventListener('click', function(){
            screen = new application.Screen();
            screen.cleanShowDiv();
	...
	show_div_add_question: function (hunt){
		...
		var buttonCreateQuestion = new application.Button(this, 'createQuestion', this.div_create_hunt, "Terminer", "btn btn-success");
		...
		buttonCreateQuestion.getButton().addEventListener('click',{
			hunt : hunt,
...
window.onload = function () {
	var database = new application.Database();
	// database.clean();
	database.init();
	screen = new application.Screen();
